...
$tabla  = $data['tabla']  ?? '';   // 'diaria' | 'materia'
$id     = intval($data['id'] ?? 0);
$accion = $data['accion'] ?? 'editar'; // 'editar' | 'eliminar'
    ...
    if ($tabla === 'diaria') {
        ...
        if ($accion === 'eliminar') {
            ...
            $pdo->prepare("
                ...
                SET estado = ?, hora_entrada = ?, observaciones = ?, dispositivo = CONCAT(dispositivo, ' [editado]')
                WHERE id_asistencia_diaria = ?
            ")->execute([$nuevoEstado, $nuevaHora, $observaciones, $id]);
