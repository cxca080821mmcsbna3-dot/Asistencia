...
$grupo      = $_GET['grupo']      ?? '';
$estado     = $_GET['estado']     ?? '';
$incidencia = $_GET['incidencia'] ?? '';   // '' | 'ninguna' | valor específico
...
try {
    if ($tipo === 'diaria') {
        ...
        if ($incidencia === 'ninguna') {
            $where[] = 'ad.incidencia IS NULL';
        } elseif ($incidencia !== '') {
            $where[] = 'ad.incidencia = :incidencia';
            $params[':incidencia'] = $incidencia;
        }
        if ($buscar) {
        ...
        $stmt = $pdo->prepare("
            SELECT ad.id_asistencia_diaria,
                   ...
                   COALESCE(ad.incidencia, '') AS incidencia
        ...
        $stmtS = $pdo->prepare("
            SELECT
                ...
                COUNT(CASE WHEN incidencia IS NOT NULL THEN 1 END)  AS con_incidencia
        ...
        // Tipos de incidencia disponibles (para el filtro dinámico)
        $stmtInc = $pdo->prepare("
            SELECT DISTINCT incidencia FROM asistencia_diaria
            WHERE incidencia IS NOT NULL AND incidencia <> ''
            ORDER BY incidencia ASC
        ");
        $stmtInc->execute();
        $tiposIncidencia = $stmtInc->fetchAll(PDO::FETCH_COLUMN);
        ...
        $stmtS = $pdo->prepare("
            ...
            FROM asistencia a WHERE a.fecha = :f
        ");
        $stmtS->execute([':f' => $fecha ?: date('Y-m-d')]);
        $stats = $stmtS->fetch(PDO::FETCH_ASSOC);
